...
func main() {
	...
	ctx, cancel := context.WithCancel(context.Background()) //context.Background() -> 빈 context 생성(기한x, 값x, 취소x)
	defer cancel()                                          // 바로 위에서 생성한 context의 취소 함수인 cancel을 이용하여 main() 종료시 context context 종료
